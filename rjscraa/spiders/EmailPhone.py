...
class EPSpider(scrapy.Spider):
    ...
    def __init__(self, content=None, *args, **kwargs):
        ...
        with open(content+'.csv') as csvfile:
            ...
            self.sites = []
            self.titles = {}
            self.linkers = LinkExtractor(
                allow=(), deny=('yahoo.com'), unique=True)
            for row in readCSV:
    ...
    def parse(self, response):
        ...
        if sitedata is None:
            url = re.sub(r"https?://(www\.)?", 'http://',
                         response.request.url, 1)
            url = url.split('.')
            try:
                sitedata = metadata(response.request.url, self.titles[
                                    url[0]][0], self.titles[url[0][1]])
            except KeyError:
                raise StopIteration
    ...
    def finder(self, response, flag):
        ...
        if flag:
            container = re.findall(
                r"((?:\d{3}|\(\d{3}\))?(?:\s|-|\.)?\d{3}(?:\s|-|\.)\d{4})", response)
        else:
            container = re.findall(
                r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,3}", response)
